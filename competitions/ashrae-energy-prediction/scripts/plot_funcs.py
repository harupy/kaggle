...
def feature_importance(features, feature_importances, importance_type, fp):
    fig, ax = plt.subplots()
    idxs = np.argsort(feature_importances)[::-1]
    y = np.arange(len(feature_importances))
    ax.barh(y, feature_importances[idxs][::-1], align='center', height=0.5)
    ax.set_yticks(y)
    ax.set_yticklabels(features[idxs][::-1])
