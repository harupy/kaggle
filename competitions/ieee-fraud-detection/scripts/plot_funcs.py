...
def metric_history(mh, fp):
    fig, ax = plt.subplots()
    for idx, data in enumerate(mh):
...
def feature_importance(features, feature_importances, importance_type, limit, fp):
    fig, ax = plt.subplots(figsize=(12, 8))
    idxes = np.argsort(feature_importances)[::-1][:limit]
    y = np.arange(len(feature_importances))[:limit]
